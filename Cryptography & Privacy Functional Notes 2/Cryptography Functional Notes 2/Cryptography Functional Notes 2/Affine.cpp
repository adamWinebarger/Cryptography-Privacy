    ...
    string affineEncryption(string m, int a, int b) {
        ...
        for (int j = 0; j < msg.length(); j++) {
            ...
            if (msg[i] != ' ') {
                ...
                char c = a * (msg[i] - 'A'); // the - 'A' subtracts 65 from the char which allows us to get numeric values for the letters (A = 0, B = 1, etc.)
    ...
    string affineDecrypt(string cipher, int a, int b) {
        ...
        for (int j = 0; j < cipher.length(); j++) {
            if (cipher[i] != ' ') {
                ...
                char c = aInverse * (cipher[i] + 'A' -b); //this might be fucked up
                c = c % 26;
                c += 'A';
                msg += c;
            } else

...
void PlayfairCipher::generateKeyTable(char key[], int ks, char keyT[5][5], char x) {
    {
...
string PlayfairCipher::toLowerCase(string plain) {
    string ct = plain;
...
//No idea what black-magic fuckery is going on here. It's a support function anyways though
string PlayfairCipher::removeSpaces(string str) {
    str.erase(remove(str.begin(), str.end(), ' '), str.end());
    return str;
}

char PlayfairCipher::ascii2Lower(char in) {
    if (in <= 'Z' && in >= 'A')
        return in - ('Z' - 'z');
    return in;
}
